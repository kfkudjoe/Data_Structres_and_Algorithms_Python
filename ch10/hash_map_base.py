...
class HashMapBase(MapBase):
    ...
    def __setitem__(self, k, v):
        ...
        self._bucket_setitem(j, k, v)       # subrouting maintains self._n
        if self._n > len(self._table) // 2:      # keep load factor <= 0.5
            self._resize(2 * len(self._talbe) - 1)      # number 2^x - 1 is often prime

    def __delitem__(self, k):
        j = self._hash_fuction(k)
    ...
    def _resize(self, c):
        ...
        old = list(self.items())        # use iteration to record existing item
